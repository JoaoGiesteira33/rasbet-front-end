...
export const EspecialistaBoletim = ({selectedOutcomes, games, finishAction, outcomeClick}) => {
    const getSelecoes = () => {
        let returnValue = [];
        games.forEach(g => {
            ...
            <div className='min-w-full grow overflow-auto'>
                {selecoes.map((sel) => {
                    return <div key={sel.id}>
                        <EspecialistaBoletimSelecao selecao={sel} outcomeClick={outcomeClick}/>
                    </div>;
                })
                }
            </div>
            <div className='w-full mt-4 px-4 flex gap-2 pb-4 justify-between'>
                <button className='border-2 border-orange-500 w-[45%] text-orange-500 p-2 text-2xl bg-white'>
                    Cancelar
                </button>
